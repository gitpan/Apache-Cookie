...
sub parse {
    ...
    foreach (@pairs) {
	my($key,$value) = split("=");
	my(@values) = map CGI::Support::unescape($_),split('&',$value);
	$key = CGI::Support::unescape($key);
...
sub new {
	...
	CGI::Support->rearrange([NAME,[VALUE,VALUES],PATH,DOMAIN,
                                 SECURE,EXPIRES],@_);
...
sub as_string {
    ...
    my($key) = CGI::Support::escape($self->name);
    my($cookie) = join("=",$key,join("&",map CGI::Support::escape($_),$self->value));
...
sub expires {
    my $self = shift;
    my $expires = shift;
    $self->{'expires'} = CGI::Support::expires($expires,'cookie') if defined $expires;
    return $self->{'expires'};
}
...
CGI::Support::Cookie - Interface to Netscape Cookies
